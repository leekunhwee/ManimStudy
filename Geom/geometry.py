...
class Line(TipableVMobject):
    ...
    # Set the attribution of start and end point -> self.start, self.end
    def set_start_and_end_attrs(self, start, end): 
        # If either start or end are Mobjects, this gives their centers, which refer to pointify 
        # 如果用Mobject来作为起止点或终止点，那么其实用的是Mobject的中心点坐标
    ...
    # pointify()通过判断输入参数是否为Mobject，来决定是否将输入参数点化处理，
    # 如果输入参数本来就是点就不必处理了。
    def pointify(self, mob_or_point, direction=None): # 实际此处的方向参数始终为None
        if isinstance(mob_or_point, Mobject): # 如果输入参数属于Mobject的实例化对象
            mob = mob_or_point 
            if direction is None:   # 无方向参数
                return mob.get_center() # 获得并返回Mobject实例化对象的中心点
            else:                   # 有方向参数
                return mob.get_boundary_point(direction)  # 依据方向参数获得并返回对应方向的边界点
        return np.array(mob_or_point) # 如果输入参数就是点坐标，则直接返回该点的坐标
